...
https-cert: ; $(sbin)/mkcert $(build)/insecure-key.pem $(build)/insecure-cert.pem

user: ; $(sbin)/mkuser _SERVER_USERNAME _SERVER_UID _SERVER_SECRET _THIS_ROOT
...
kernel-development-no-build: ; docker rm _DOCKER_CONTAINER 2> /dev/null ; docker run -d -v _THIS_ROOT:_SERVER_ROOT -p _THIS_PORT:_SERVER_PORT -p _THIS_SECURE_PORT:_SERVER_SECURE_PORT ifelse(_SSH_PORT, `', `', `-p '_THIS_SSH_PORT`:'_SERVER_SSH_PORT) --name _DOCKER_CONTAINER _DOCKER_IMAGE $(bin)/development kernel

kernel-boot: docker-build kernel-boot-no-build ; 
kernel-boot-no-build: ; docker rm _DOCKER_CONTAINER 2> /dev/null ; docker run -d -v _THIS_ROOT:_SERVER_ROOT -p _THIS_PORT:_SERVER_PORT -p _THIS_SECURE_PORT:_SERVER_SECURE_PORT ifelse(_SSH_PORT, `', `', `-p '_THIS_SSH_PORT`:'_SERVER_SSH_PORT) --name _DOCKER_CONTAINER _DOCKER_IMAGE $(bin)/launch kernel
...
install: ; $(sbin)/install _LOCAL_PUBLIC_KEY _LOCAL_PRIVATE_KEY _LOCAL_SECURE_KEY _LOCAL_SECURE_CERT _LOCAL_CONFIG _HOST_USER`@'_HOST_ADDR _HOST_REPOSITORY _HOST_HOME _HOST_SECURE_KEY _HOST_SECURE_CERT _HOST_CONFIG _HOST_ROOT
install-clean: ; $(sbin)/install _LOCAL_PUBLIC_KEY _LOCAL_PRIVATE_KEY _LOCAL_SECURE_KEY _LOCAL_SECURE_CERT _LOCAL_CONFIG _HOST_USER`@'_HOST_ADDR _HOST_REPOSITORY _HOST_HOME _HOST_SECURE_KEY _HOST_SECURE_CERT _HOST_CONFIG _HOST_ROOT yes

install-shell: ; $(sbin)/install-shell _HOST_FQDN _SHELL_USER _SHELL_HOME _SHELL_BIN _SHELL_PORT _SHELL_PATH _LOCAL_PRIVATE_KEY _SHELL_SECURE_KEY _SHELL_SECURE_CERT _HOST_USER`@'_HOST_ADDR _HOST_SSH_PORT
...
poke: ; $(sbin)/poke _LOCAL_PRIVATE_KEY _HOST_USER`@'_HOST_ADDR _HOST_HOME _HOST_ROOT
clean: ; rm $(build)/*
