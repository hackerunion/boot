...
kernel-boot-no-build: ; docker rm _DOCKER_CONTAINER 2> /dev/null ; docker run -d -v _THIS_ROOT:_SERVER_ROOT -p _THIS_PORT:_SERVER_PORT ifelse(_SSH_PORT, `', `', `-p '_SSH_PORT`:'_SSH_PORT) --name _DOCKER_CONTAINER _DOCKER_IMAGE ./boot/bin/launch kernel
...
kernel-running: ; docker inspect --format="{{ .State.Running }}" | grep -qi 'true'
